...
async function seedDatabase() {
    ...
    db.users.push({ id: adminId, name: 'Admin User', email: 'admin@platform.com', password_hash: adminHash, role: 'admin', status: 'active', phone: '', avatar_url: '', created_at: new Date().toISOString() });
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        db.users.filter(u => u.role === 'admin').forEach(a => {
            db.notifications.push({ id: uuidv4(), user_id: a.id, title: 'New Registration', message: `${name} (${email}) needs approval`, type: 'approval', is_read: false, created_at: new Date().toISOString() });
        });
...
app.put('/api/admin/users/:userId/approve', authenticate, authorize('admin'), (req, res) => {
    ...
    if (!user) return res.status(404).json({ error: 'User not found' });
    user.status = 'active';
    db.notifications.push({ id: uuidv4(), user_id: user.id, title: 'Account Approved', message: 'Your account has been approved', type: 'approval', is_read: false, created_at: new Date().toISOString() });
...
app.post('/api/admin/experts/create', authenticate, authorize('admin'), async (req, res) => {
    try {
        ...
        db.users.push({ id: expertId, name, email, password_hash: hash, role: 'expert', status: 'active', phone: phone || '', avatar_url: '', created_at: new Date().toISOString() });
        db.expertProfiles.push({ id: uuidv4(), user_id: expertId, expertise: expertise || [], bio: bio || '', hourly_rate: parseFloat(hourly_rate) || 0, total_earnings: 0, available_balance: 0, application_status: 'approved', approved_at: new Date().toISOString(), approved_by: req.user.id, is_suspended: false, average_rating: 0, total_reviews: 0, is_active: true });
        db.expertEarningsSummary.push({ id: uuidv4(), expert_id: expertId, total_earned: 0, total_commission_paid: 0, available_balance: 0, pending_payout: 0, total_paid_out: 0, is_frozen: false });
        
        db.notifications.push({ id: uuidv4(), user_id: expertId, title: 'Welcome!', message: 'Your expert account is ready', type: 'approval', is_read: false, created_at: new Date().toISOString() });
...
app.post('/api/admin/experts/:expertId/suspend', authenticate, authorize('admin'), (req, res) => {
    ...
    db.expertSuspensions.push({ id: uuidv4(), expert_id: user.id, suspended_by: req.user.id, suspension_type: req.body.suspension_type || 'temporary', reason: req.body.reason || 'Admin suspension', start_date: new Date().toISOString(), is_active: true });
    db.notifications.push({ id: uuidv4(), user_id: user.id, title: 'Account Suspended', message: `Your account has been suspended. Reason: ${req.body.reason || 'Admin action'}`, type: 'system', is_read: false, created_at: new Date().toISOString() });
...
app.post('/api/admin/experts/:expertId/reactivate', authenticate, authorize('admin'), (req, res) => {
    ...
    db.notifications.push({ id: uuidv4(), user_id: user.id, title: 'Account Reactivated', message: 'Your account has been reactivated', type: 'approval', is_read: false, created_at: new Date().toISOString() });
...
app.post('/api/admin/commission-config', authenticate, authorize('admin'), (req, res) => {
    ...
    db.commissionConfig.push(config);
...
app.post('/api/expert/services', authenticate, authorize('expert'), (req, res) => {
    ...
    db.expertServices.push(service);
    res.status(201).json({ service });
});

app.put('/api/expert/availability', authenticate, authorize('expert'), (req, res) => {
    db.expertAvailability = db.expertAvailability.filter(a => a.expert_id !== req.user.id);
    const schedule = req.body.schedule || [];
    schedule.forEach(s => {
        db.expertAvailability.push({ id: uuidv4(), expert_id: req.user.id, day_of_week: s.day, start_time: s.start, end_time: s.end, is_available: true, recurrence: s.recurrence || 'weekly' });
    });
...
app.post('/api/user/consultations', authenticate, authorize('user'), (req, res) => {
    ...
    db.users.filter(u => u.role === 'admin').forEach(a => {
        db.notifications.push({ id: uuidv4(), user_id: a.id, title: 'New Consultation', message: `${req.user.name} requested: ${req.body.title}`, type: 'system', is_read: false, created_at: new Date().toISOString() });
    });
...
app.post('/api/user/events/:eventId/enroll', authenticate, authorize('user'), (req, res) => {
    ...
    db.enrollments.push({ id: uuidv4(), user_id: req.user.id, event_id: eventId, enrollment_status: 'enrolled', enrolled_at: new Date().toISOString() });
...
app.post('/api/consultations/:id/messages', authenticate, (req, res) => {
    ...
    db.consultationMessages.push(message);
...
seedDatabase().then(() => {
    server.listen(PORT, () => {
        ...
        console.log(`🔐 Default credentials: admin@platform.com / admin123`);
    });
});
